...
struct point {
    ...
    bool operator < (const point &other) const {
        ...
        else if (other.y != y) return other.y < y;
        else return other.z < z;
    }
};
...
void solve ()
{
    ...
    while (!q.empty()) {
        ...
        for (int i = 1; i <= m; i++) {
            ...
            int yy = u.y + b[i].y;
            int zz = u.z + b[i].z;
            if (mp1.find({xx, yy, zz}) != mp1.end()) continue;
            if (mp[{xx, yy}].empty()) continue;
            auto it = mp[{xx, yy}].lower_bound(zz);
            if (it == mp[{xx, yy}].begin()) {
                continue;
            }
            it--;
            if (mp1.find({xx, yy, *it + 1}) != mp1.end()) continue;
            mp1[{xx, yy, *it + 1}] = true;
