...
void solve ()
{
    ...
    i64 ans = 0;
    for (int i = 1; i <= n; i++) {
        int rk = lower_bound(v.begin() + 1, v.end(), a[i]) - v.begin();
        i64 t = a[i] % mod;

        i64 count = (1 + cnt.sum(rk - 1)) % mod;
        i64 res = (val.sum(rk - 1) % mod + count * t % mod) % mod;

        cnt.point_add(rk, count);
        val.point_add(rk, res);
